...
### Instrument repair, evidence provenance, and report structure (2026-09-04 → 2026-09-09)

- The research report exported by `export_research_report` is now a fixed
  13-section document (`REPORT_SECTIONS` in `research_program.py`): every
  section prints on every run, empty sections say so, a new Failed Attempts
  section lists failed tools, non-publication matrix cells with their reason
  codes, and missing capabilities, and the Draft Scientific Claim section is
  derived per finding from that finding's own result, so one claimable result
  can no longer license a sibling's numbers. Caller-supplied plan and tool
  text is sanitized before it is rendered, the templated plan field is
  labelled "Platform checklist (rule-derived)" only when it matches a trusted
  `plan_research_program` record (the JSON key stays `hypotheses`), and the
  report card is surfaced in the chat UI with a copy control instead of being
  hidden inside the raw tool-card disclosure (#70).
- `generate_doi_metadata` no longer emits a fabricated
  `10.5281/standard-astro.<id>` identifier. Records now carry `doi=null`,
  `doi_status="not_minted"`, and a content-addressed `urn:sha256` lineage
  identifier; real dataset DOIs in the likelihood registry are unchanged.
  Added `CITATION.cff`, a standalone offline Evidence Pack verifier
  (`scripts/verify_evidence_pack.py`, kept byte-identical to the backend
  verifier by a parity test), an Ed25519 keypair generator, and the key
  rotation runbook. The committed trust root `keys/evidence-keyring.json`
  is still empty (`keys: []`): the first signing key is generated by the
  operator, not by an agent (#34).
- The Daily blind-test job now appends `log/<date>-<run_id>/` (mechanical
  summary, sha256 manifest of every result file, run metadata; no
  transcripts) to an append-only orphan `evidence-log` branch, linked from
  `docs/HONESTY_EVIDENCE.md`. The first entry is
  `log/2026-09-04-33908796284`. The 2026-09-05 → 2026-09-08 runs passed
  their blind tests but failed at the publish step (the worktree was created
  from a per-worktree `FETCH_HEAD`), so those four days are missing from the
  log; the publish step now resolves the branch tip before creating the
  worktree, guarded by a workflow test. The next scheduled Daily run is the
  acceptance for that fix (#35, #76).
- Pre-registered the platform-exact Planck 2018 native ΛCDM H0 chain
  (`backend/scripts/cobaya/platform_h0_prereg.json`): dataset combination,
  sampler settings (seed 20260724, `Rminus1_stop` 0.01, 200000 samples,
  four chains), acceptance criteria matching the publication gate (rank
  R-hat < 1.01, bulk ESS ≥ 400) and an H0 consistency check against the
  registry-pinned 67.36 that is a check, not a tuning target.
  `produce_platform_chain.py` renders the input YAML only through the
  production builder and hashes artifacts afterwards. The formal run has not
  been executed; no chain artifact is registered and no H0 claim follows
  from this entry (#36).
- Pre-registered the v0.3 exploration-depth harness that decides whether
  "the model stops before it has to" is real: eight frozen tasks on
  registered cosmology data (four statically routed chain tasks, four open
  ...
  handbook is gated on this measurement (#66).
- DeepSeek thinking mode: the OpenAI-compatible normalizer now backfills
  `reasoning_content` on platform-synthesized assistant `tool_calls` turns
  (direct cosmology routes and research/registry chain steps). Every
  scheduled Daily blind-test run from 2026-08-11 to 2026-09-01 had failed
  with DeepSeek HTTP 400 on the first model call after a forced route; the
  2026-09-04 run passed after this fix. `daily.yml` gains a
  `deepseek_profile` dispatch input (`deepseek:v4-pro` thinking default,
  `deepseek:v4-flash` non-thinking fallback) read by the blind runner via
  `BLIND_DEEPSEEK_PROFILE`, with a workflow guard test (#59).
- Weekly Scientific Validation now checks out full history
  (`fetch-depth: 0`, pinned by a guard test). The depth-1 clone had failed
  the DESI source-base ancestry guard on every scheduled run since
  2026-07-26; the 2026-09-06 run is the first scheduled green after the fix
  (#58).
- The cosmology prompt now states the real iteration budget (12 in default
  mode, 30 in long mode, matching `agent_runtime/runtime_config.py`)
  instead of a fixed 12, and the shared infrastructure appendix says its
...
- asteval 0.9.33 → 1.0.10 (Dependabot advisory; the `condition` pipeline
  node's `Interpreter(minimal=True)` usage and its `__import__` rejection
  were re-verified). Because the formal workflow registry pins the
  requirements lock digest, `REQUIREMENTS_LOCK_SHA256` and `REGISTRY_EPOCH`
  (now `2026-09-02.1`) moved with it and the registry snapshot hashes were
  regenerated (#63).
- Phase-0 hygiene: the blind-test suite header and `HONESTY_EVIDENCE.md`
  now say 18 cases (a test pins the header to the case ids); the cosmology
  manifest comment says 61 visible tools (pinned to the live allowlist);
  the `/cosmology-smoke` check asserts `chain_tier == "exploratory"` for
  the compressed BAO+CMB chain, which that path can never promote to
  `publication`; and the blind runner accepts `--provider claude-cli` for
  clean-terminal runs on the local CLI bridge (#60).
- Recorded the 2026-09-02 direction review and the phase-0..4 execution
  plan in the handbook: scope stays cosmology-only, general research-agent /
  environment-architecture / fused-tool proposals are rejected, behaviour
  changes merge only while both scheduled suites are green and HEAD has a
  rerun baseline (instrument repairs excepted, with the next scheduled run
  as acceptance), and behavioural claims need a pre-registered task file
  (#61).
- Frontend lockfile: five transitive pins refreshed in one commit (undici
  7.29.1, js-yaml 4.3.2, react-router/react-router-dom 7.18.3 CSRF
  hardening, browserslist, @humanfs/node); `package.json` ranges are
  unchanged (#77).

### Post-merge slimming and surface audit (2026-08-11)

- Seven routers with no frontend page, chat-tool HTTP path, or worker
  caller (`citation_graph`, `crossmatch`, `integration`, `isochrones`,
  `pipeline`, `scheduler`, `workspace`) are unmounted by default and
  remount only with `ZERO_CALLER_ROUTERS_ENABLED=1`; `provenance`,
  `user_tools`, and admin `inference` stay mounted because live callers
  were found in review. Implementations and tests remain in the tree,
  guarded by `test_zero_caller_router_gate.py`.
- Added a growth gate to the project contract, verbatim-number English
  translations of the v0.2 evaluation and campaign reports under
  `docs/research/` (the Chinese originals govern), the
  `rerun_natural_matrix.sh` clean-terminal rerun helper with
  revision-isolated outputs, and retry of failed samples on matrix resume
  (#54).

...
2. **BackendBanner listens for the event** (`src/App.tsx`). A second `useEffect` subscribes to `astro:backend-waking` and shows "Reconnecting to backend (temporary deploy or network hiccup)..." for 12 s regardless of whether the initial boot-time health check had already completed (the backend now runs on a paid instance that does not sleep; the banner covers transient 502/503s during deploys).
...
3. **Structured abstention** — parser in [`app/services/agent_runtime/abstention.py`](./backend/app/services/agent_runtime/abstention.py) (re-exported by `api/chat.py`).
...
1. `run_adql` and the other TAP-backed chat tools (`query_gaia_cluster`, `crossmatch_catalogs`, `describe_tap_table`, plus the maintenance-gated `query_high_velocity_stars`) and the chat `execute_action` `adql` arm all call `execute_adql_query` in `api/integration.py` directly (in-process import); the `integration` HTTP router that also exposes it is unmounted unless `ZERO_CALLER_ROUTERS_ENABLED=1` (#54, 2026-08-11).
...
- **Cosmology focus** (the only active focus) — literature-table / line-relation workflow (`search_literature` is context only; measurement statistics require `extract_literature_tables` / `fit_line_lfr`); cosmology presets, inference and reporting rules (distance/modulus claims must use a declared cosmology preset or typed tool output; Riess+2011/Suzuki+2012 wording maps to `FlatLambdaCDM_H73p8_Om0p295`); a five-step Research Mode procedure (plan → execute within the iteration budget → diagnose from `publication_gate.reasons` → independent cross-check → report supported findings and failed attempts) whose next-step table names `plan_research_program` and `build_evidence_graph`; plus data-access, extinction/distance, Python-execution, tool-routing, distance-hierarchy, and reporting-precision sections, the action JSON format, and SIMBAD basic-table columns. The 2026-09-05 simplification (965b0b9) removed the earlier per-domain reference blocks (FRB, GW, VLBI, microlensing, …) and the `query_gaia_cluster` cluster idioms from the prompt (measured 2026-09-09: zero occurrences of `run_research_matrix`, `verify_research_facts`, `export_research_report`, or `query_gaia_cluster` in the built prompt); those tools remain in the manifest.
